...
Ext.define('Ext.ux.Fileup', {
            ...
            children: [
                {
                    ...
                    tabindex: -1,
                    hidden: false,
                    style: 'opacity:0;position:absolute;top:-3px;right:-3px;bottom:-3px;left:-3px;z-index:16777270;width:100%;'
                }
            ]
        }
    ],
    ...
    currentState: null,
    //captionBrowse:	'Browse',
	//captionReady: 'Upload',
	//captionUploading: 'Uploading',
    ...
    initialize: function() {
        ...
        me.fileElement.dom.onchange = function() {
        	try{
	        	//console.log("fileelement change");
        ...
        console.error('change state to browse');
    },
    
    // @private
    onButtonTap: function() {
        var me = this;
        console.error(me.currentState);
    ...
    doPost: function() {
    	...
    	try{
    		if (me.currentState == 'ready') {
    			//console.error(me.fileElement.dom.files);
		    	var file = me.fileElement.dom.files[0];
		    	me.fireEvent('uploadstart', file);
	            me.doUpload(file);
		    	return file;
...
//            }
            ...
            switch (state) {
                case 'browse':
                	me.setText(me.getCaptionBrowse());
                    me.currentState = 'browse';
                    me.reset();
                    break;
                    
                case 'ready':
                	me.setText(me.getCaptionReady());
                ...
                case 'uploading':
               	 	me.setText(me.getCaptionUploading());
                    me.currentState = 'uploading';
                    break;
            }
        } else {
        	console.error('Config for FileUp state "'+ state +'" not found!');
            // <debug>
                ...
                if (this.readyState == 4) {
                    if(this.status == 200) {
                        ...
                        if (response && response.success) {
                            ...
                            me.fireEvent('success', response.path, response, this, e);
                        } else if (response && response.message) {
                            // Failure
                        	console.warn('failure'+response.message);
                            me.fireEvent('failure', response.message, response, this, e);
                        } else {
                            // Failure
                        	console.warn('failure'+response.message);
                            me.fireEvent('failure', 'Unknown error'+response.message, response, this, e);
                        }
