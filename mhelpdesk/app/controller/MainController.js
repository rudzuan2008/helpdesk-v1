...
//				},
				show : function(thisPage, eOpts) {
					...
					try {
						...
						// var delayRefresh = Ext.create('Ext.util.DelayedTask',
						// function() {
						// if (me._isPlaying) {
						// me.stopSound();
						// }
						...
						// }, me);
						//
						// delayRefresh.delay(1000);
//						me.doDelay(1000, function() {
//									if (me._isPlaying) {
//										me.stopSound();
//									}
//									me.hideMyMasked();
//								});
//						if (thisPage.getTitle()=="") {
...
//						}
	...
	showMyMasked : function(msg, autoHide) {
	...
	setupData : function() {
		...
		try {
			...
			var data = [{
						...
						"duration" : 1,
						"status" : 'ACTIVE',
						"description" : 'Subject Matematik I',
						"subject" : 'Matematik I',
						"code" : '001',
						"indicator" : 'A',
						"locale" : 'en',
			...
			// me._localDailyTimetableStore.removeAll();
			// me._localDailyTimetableStore.sync();
			// me._localDailyTimetableStore.load();

			me.clearStore(me._localDailyTimetableStore);
			me._localDailyTimetableStore.add(data);
			me._localDailyTimetableStore.sync();
			// me._localDailyTimetableStore.load({
			// callback : function() {
			// me._localDailyTimetableStore.sort("stime", "ASC");
			// }
			// });
	...
	launch : function(app) {
		...
		try {
			...
			window.addEventListener('popstate', function () {
		        var portal = me.getMainpage();  // won't have portal until app is initialized
		        if (portal) {
		        	console.error(portal.getActiveItem().getItemId());
		        	var container = me.getMain();
		        	var prevPage = container.getActiveItem().getItemId();
		        	if (portal.getActiveItem().getItemId() != container.getActiveItem().getItemId()) {
//		        		container.pop();
//		        		var currPage = container.getActiveItem().getItemId();
//		        		console.error(container.getActiveItem().getItemId());
...
//		            }
		    ...
		    function displayPage() {
				...
				//var homePage = me.getHomepage();
				//homePage.setTitle(system.getTitle());
				//me.showMyMasked(locale.getText('msgWaitPage'),true);
				
		    }
			var mainApp = function(proceed){
				me.showMyMasked(locale.getText('msgWaitPage'),false);
				//console.error("proceed?"+proceed);
				if (proceed) {
...
//						me.changeCSS('resources/css/'+theme, oldlink);
//					}else{
...
//					console.error(oldlink);
					...
					me.load_config(false, function(records, operation, success) {
						//console.error(records.length);
						//console.error(records);
						var recordConfig = records[0];
						//console.error(recordConfig);
						
						if (recordConfig) {
							var email_id =  recordConfig.get('default_email_id');	
							var title = recordConfig.get('helpdesk_title');
							
							me.getEmail('email_id='+email_id,function(records, operation, success){
								
								if (success) {
									var recordEmail = records[0];
									//console.error(recordEmail);
									system.setTitle(recordConfig.get('helpdesk_title'));								
									system.setEmail(recordEmail.get('email'));
									system.setPhone(recordEmail.get('phone'));
									system.setCompany(title);
									me.updSysData("title",recordConfig.get('helpdesk_title'));
									me.updSysData("email",recordConfig.get('email'));
									me.updSysData("phone",recordConfig.get('phone'));
					...
					me.debugHandler(locMod, "log", "members:", system.getMembers());
...
//						me.updSysSetting("theme","app.css");
//					}
					me.debugHandler(locMod, "log", "screenMode:", system.getTheme());
				}else{
					me.showMyMasked(locale.getText('msgWaitPage'),false);
